...
USER = sys.argv[1] if len(sys.argv) > 1 else "sktamalik"
...
def get_data(user):
    ...
    try:
        ...
        if os.path.exists(here):
            ...
            # remap to jogruber shape
            return {
                "contributions": [{"date": d["date"], "count": d["count"], "level": min(d["count"], 4) if d["count"] > 0 else 0} for d in raw["days"]],
