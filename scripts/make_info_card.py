"""
Build a neofetch-style info card SVG (Andrew6rant style) to sit beside
the ASCII portrait: colored key/value rows for work, stack, and highlights.

Static content, hand-authored below. Lines fade/slide in on a short stagger.
STATIC=1 emits frozen state for Quick Look previews.
"""
import html
import os
...
ROWS = [
    ("host",),
    ("kv", "Now",   "Freelance Developer"),
    ("kv", "Prev",  "General Service Analyst"),
    ("kv", "Prev",  "General Affair Admin"),
    ("kv", "Edu",   "S1 Teknik Informatika (3.83/4.00)"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Web",   "HTML, CSS, JS, PHP, Laravel, WordPress"),
    ("kv", "Script","Google Apps Script, Python"),
    ("kv", "Cloud", "Microsoft Azure (MCF), AWS"),
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">sktamalik@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">sktamalik</tspan><tspan fill="{MUTED}">@</tspan>'
