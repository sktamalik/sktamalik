"""
Convert a portrait photo into a monochrome ASCII-art SVG (Andrew6rant
style) that "types" itself in like a terminal, then holds.
...
Each row is revealed with a left-to-right clip wipe plus cursor.
"""
...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "sktamalik-ascii.svg")
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">sktamalik@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'sktamalik@github:~$ whoami <tspan fill="{INK}">SktaMalik_</tspan></text>')
parts.append(f'<rect x="{PAD+172}" y="{status_y-12:.1f}" width="8" height="14" fill="{INK}">'
